...
| — | `profile_locale` (`20260906130000`) | i18n agent: adds `profiles.locale` (nullable, checked against `'tr'`/`'en'`), the strongest signal in the locale resolution order (`src/i18n/locale.ts`: profile column, then `akinti_locale` cookie, then `Accept-Language`, default `tr`). Self-service via `updateProfileLocale` (`src/lib/db/profiles.ts`), covered by the existing `profiles_update_own` RLS policy; no guard-trigger change needed. |
| — | `rate_limit_actions_union` (`20260906140000`) | Bug fix (review3 #1): two same-day migrations (`subscriptions`, `flow`) each rewrote `rate_limit_events_action_known` from scratch with only their own new action, so applying in filename order silently dropped `'billing_checkout'` and every Pro checkout failed the check constraint after the provider had already created a real checkout. Replaces the check constraint with a `rate_limit_actions` lookup table (closed whitelist via FK, not a re-appended `in (...)` list) seeded with the full union: `comment`, `follow`, `message`, `duet_request`, `share`, `report`, `audio_upload`, `challenge_entry`, `billing_checkout`, `flow_event`. `rate_limit_events.action` now FKs to it (`on update cascade`) instead of the dropped check constraint. |
| — | `enqueue_audio_job_guard` (`20260906150000`) | Security fix (review3 #3, #4): closes two independent client-side bypasses of the AKINTI Pro gate on `pitch_snap`/`self_harmony`. (3) `enqueue_audio_job()` is `authenticated`-grantable and only checked asset ownership, so a free account could call the RPC directly with a Pro-only preset and the worker would process it — now re-checks `has_pro(owner)` itself (service requests exempt). (4) `audio_assets_guard_update` (migration 42) omitted `enhancement_preset` from its guarded columns, so a direct PostgREST PATCH could swap a `natural` asset to a Pro preset post-creation — the guard now also locks `enhancement_preset` once `processing_status = 'ready'`, and requires `has_pro` to set a Pro-only value before then. |
| — | `billing_event_ordering` (`20260906160000`) | Bug fix (review3 #7): adds `subscriptions.last_event_at` (the `occurredAt`/`iyziEventTime` of the most recently *applied* webhook event) so `applyBillingEvent` (`src/lib/billing/repository.ts`) can detect and skip a strictly-older, out-of-order retry instead of overwriting newer state — the event is still recorded in `billing_events` (audit trail) either way. |
| — | `count_flow_new_bound` (`20260906170000`) | Performance fix (review3 #13): `count_flow_new()` did a full unbounded, RLS-filtered scan of `waves` on every nav-shell render. Re-created bounded to Waves published in the last 7 days with a `limit 100` inner query (`CountBadge` already renders `99+` past its own cap), backed by a new partial index `waves_published_recent_idx (published_at desc) where deleted_at is null`. |
| — | `revoke_has_pro_anon` (`20260906181700`) | Security fix (review3 #36): `has_pro(uuid)` was grantable to `anon`, letting a signed-out caller probe any user id's Pro status. Revokes the `anon` execute grant; `authenticated`/`service_role` are unaffected — the Pro badge is only ever rendered server-side. |
| — | `flow_dedupe_invitations` (`20260906190000`) | Bug fix (review3 #18): `get_flow_page()`'s `invitations` CTE excluded already-shown Waves but was never anti-joined against `ranked` (the followed/duet/challenge/rising candidates), so a Wave qualifying for both could appear twice in the same page (a React duplicate-key collision in `FlowScreen`, keyed on `wave.id`). `create or replace`s the full function body to also exclude anything already present in `ranked` from `invitations`. |
| — | `flow_cursor_slot` (`20260906200000`) | Bug fix (review3 #31), building on the previous migration: `get_flow_page()` always advanced `cursor_slot` by the full `v_limit` regardless of how many rows the call actually emitted, so once the invitation pool or ranked stream ran short, the "every 8th absolute position" invitation phase drifted from what the reader actually saw and later skipped invitations that were never shown. Re-creates the function to advance `v_slot` by the count of rows actually emitted (`page`/`emitted` CTEs) instead. |
| — | `flow_impressions_wave_idx` (`20260906210000`) | Performance fix (review3 #30): `flow_impressions.wave_id` (`on delete cascade` from `waves`) had no index of its own, so every Wave deletion sequentially scanned the whole table to find cascade rows. Adds `flow_impressions_wave_idx (wave_id)` and `prune_flow_impressions(interval default 30 days)` (service-role-only, same shape as `prune_rate_limit_events()`; not yet called by anything — a worker/pg_cron job would call it periodically). |
