-- review3 finding 31: `get_flow_page`'s final select always returned
-- `(v_slot + v_limit)` as `cursor_slot`, regardless of how many rows this
-- call actually emitted. Once the invitation pool is exhausted, or the
-- ranked stream runs short (near the end of the feed), the slot counter
-- still advances by the full `v_limit`, so the "every 8th absolute
-- position" invitation phase drifts away from what the reader actually
-- saw, and future invitation offsets skip invitations that were never
-- shown.
--
-- Fix: compute the number of rows actually emitted (the `page`/`emitted`
-- CTEs) and advance `v_slot` by that instead. Full function body
-- re-created (`create or replace`, no signature change), building on the
-- finding-18 fix in 20260906190000_flow_dedupe_invitations.sql.
...
begin
  ...
  page as (
    -- The actual emitted rows for this page: fewer than v_limit whenever
    -- the ranked stream or the invitation pool ran short of a full slot.
    select
      m.abs_pos,
  ...
  select
    ...
    lr.cursor_score,
    lr.cursor_id,
    -- review3 finding 31: advance by the number of rows THIS call actually
    -- emitted, not always the full v_limit — otherwise, once the invitation
    -- pool or the ranked stream runs short, the slot counter drifts ahead
    -- of what the reader actually saw and the "every 8th absolute
    -- position" invitation phase skips invitations that were never shown.
    (v_slot + e.n) as cursor_slot
  from page p
  cross join last_raw lr
  cross join emitted e
  order by p.abs_pos;
end;
$fn$;
...
comment on function public.get_flow_page(jsonb, integer, integer) is
  ...
  'by the ranked stream never also fills an invitation slot (review3 '
  'finding 18). `cursor_slot` advances by the rows actually emitted, not '
  'always the full limit (review3 finding 31).';
