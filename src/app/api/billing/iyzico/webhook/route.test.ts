...
function makeRequest(body: string): NextRequest {
  return new NextRequest("http://localhost/api/billing/iyzico/webhook", { method: "POST", body });
}
...
vi.mock("@/lib/billing/iyzico", () => ({
  iyzicoProvider: { provider: "iyzico" },
}));
...
describe("POST /api/billing/iyzico/webhook", () => {
  ...
  it("returns 200 once the event is durably recorded", async () => {
    ...
    const response = await POST(makeRequest(JSON.stringify({ iyziEventType: "subscription.order.success" })));
